...
class AuthController {
    ...
    public function login(): void {
        ...
        try {
            ...
            // Vérification du rôle sélectionné vs rôle réel en base
            $selectedRole = trim($_POST['role'] ?? '');
            if (!empty($selectedRole) && in_array($selectedRole, ['admin', 'medecin', 'patient'], true)) {
                if ($user['role'] !== $selectedRole) {
                    $_SESSION['error'] = "Rôle incorrect pour ce compte. Veuillez sélectionner le bon rôle.";
                    header('Location: ' . $this->getBaseUrl() . 'index.php?page=login');
                    exit;
                }
            }
