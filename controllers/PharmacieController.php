...
class PharmacieController {
    ...
    /**
     * Chatbot IA — appelle l'API Google Gemini (GRATUIT).
     * 1500 requêtes/jour offertes, aucune carte bancaire requise.
     * Clé à obtenir sur : https://aistudio.google.com
     * Le modèle est limité à conseiller uniquement les produits du catalogue.
     * Retourne [string $message, array $suggestions].
     */
    private function chatbotRecommanderProduits(string $query): array
    {
        // 1. Charger tous les produits actifs pour le contexte
        ...
        // 2. Construire un catalogue simplifié à envoyer à l'IA
        $catalogueLines = [];
        foreach ($produits as $p) {
            $catalogueLines[] = sprintf(
                '[ID:%d] %s | Catégorie: %s | Prix: %.2f TND | Stock: %d | %s',
                (int)$p['id'],
                $p['nom'] ?? '',
                $p['categorie_nom'] ?? 'N/A',
                (float)($p['prix_vente'] ?? 0),
                (int)($p['stock'] ?? 0),
                mb_substr($p['description'] ?? '', 0, 120)
            );
        }
        $catalogueTexte = implode("\n", $catalogueLines);

        // 3. Construire le prompt complet pour Gemini
        // (Gemini intègre le system prompt directement dans le message)
        $promptComplet =
            "Tu es un conseiller en parapharmacie virtuel pour la boutique Valorys.\n"
            . "Ton unique rôle est d'aider les clients à trouver des produits adaptés à leur besoin\n"
            . "parmi le catalogue ci-dessous. Tu ne dois JAMAIS recommander de produits qui ne figurent\n"
            . "pas dans ce catalogue, ni donner de conseils médicaux, ni traiter d'autres sujets.\n\n"
            . "CATALOGUE DES PRODUITS DISPONIBLES :\n"
            . $catalogueTexte . "\n\n"
            . "RÈGLES ABSOLUES :\n"
            . "- Réponds uniquement en rapport avec les produits du catalogue ci-dessus.\n"
            . "- Si la question ne concerne pas la parapharmacie ou nos produits, réponds :\n"
            . "  'Je suis uniquement là pour vous conseiller sur nos produits de parapharmacie.'\n"
            . "- Identifie les IDs des produits pertinents et indique-les sous la forme JSON à la fin\n"
            . "  de ta réponse, dans un bloc <ids>[1,2,3]</ids> contenant jusqu'à 3 IDs.\n"
            . "- Sois bref, bienveillant et professionnel. Réponds en français.\n\n"
            . "Question du client : " . $query;

        // 4. Appel à l'API Google Gemini via cURL
        ...
        // 5. Gérer les erreurs réseau / API
        if ($curlError || $httpCode !== 200 || !$response) {
            return $this->chatbotFallback($query, $produits);
        }

        $data = json_decode($response, true);
        // Structure réponse Gemini : candidates[0].content.parts[0].text
        $texteIA = $data['candidates'][0]['content']['parts'][0]['text'] ?? '';

        if (empty($texteIA)) {
            return $this->chatbotFallback($query, $produits);
        }

        // 6. Extraire les IDs suggérés par le modèle
        $suggestions = [];
        if (preg_match('/<ids>(\\[[\\d,\\s]*\\])<\/ids>/', $texteIA, $m)) {
            $ids = json_decode($m[1], true);
            if (is_array($ids)) {
                $produitIndex = array_column($produits, null, 'id');
                foreach (array_slice($ids, 0, 3) as $id) {
                    $id = (int)$id;
                    if (isset($produitIndex[$id])) {
                        $suggestions[] = $produitIndex[$id];
                    }
                }
            }
        }
