...
private function setFlash(string $type, string $message): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                if (!empty($_FILES['image']['name'])) {
                    $image = $this->uploadEventImage($_FILES['image']);
                    if (!$image) {
                ...
                if (empty($errors)) {
                    $data = [
                        ...
                        'status' => $_POST['status'],
                        'sponsor_id' => !empty($_POST['sponsor_id']) ? (int)$_POST['sponsor_id'] : null,
                        'image' => $image,
                    ];
                    ...
                    if ($eventModel->create($data)) {
                        ...
                        $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la création de l\'événement.'];
                    }
                }
            }

            // Si erreurs, sauvegarder et recharger le formulaire
            if (!empty($errors)) {
                $_SESSION['errors'] = $errors;
    ...
    public function updateEvent(int $id): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                // Traitement de l'upload d'image si présente
                $image = null;
                if (!empty($_FILES['image']['name'])) {
                    $image = $this->uploadEventImage($_FILES['image']);
                    if (!$image) {
                    ...
                    // Garder l'image existante si pas de nouvelle
                ...
                if (empty($errors)) {
                    ...
                    if ($eventModel->update($id, $data)) {
                        ...
                        $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la mise à jour de l\'événement.'];
                    }
                }
            }
    ...
    // Méthode de validation des données d'événement
    private function validateEventData(array $data): array {
        ...
        } elseif (strlen(trim($data['lieu'])) < 2) {
            $errors['lieu'] = 'Le lieu doit contenir au moins 2 caractères.';
        }

        // Validation de la capacité
        if (empty($data['capacite_max'] ?? '')) {
            $errors['capacite_max'] = 'La capacité maximale est obligatoire.';
        } else {
            $capacite = (int)$data['capacite_max'];
            if ($capacite <= 0) {
                $errors['capacite_max'] = 'La capacité doit être supérieure à 0.';
            } elseif ($capacite > 100000) {
                $errors['capacite_max'] = 'La capacité ne doit pas dépasser 100 000 personnes.';
            }
        }
        ...
        if (empty($data['status'] ?? '')) {
            ...
            $validStatus = ['à venir', 'en_cours', 'terminé', 'annulé'];
    ...
    // Méthode pour télécharger l'image d'un événement
    private function uploadEventImage(array $file): ?string {
        $maxFileSize = 5 * 1024 * 1024; // 5 MB
        $uploadDir = __DIR__ . '/../uploads/articles/'; // Réutiliser le dossier articles pour les événements
        ...
        $filename = 'event_' . time() . '_' . uniqid() . '.' . pathinfo($file['name'], PATHINFO_EXTENSION);
    ...
    public function deleteEvent(int $id): void {
        ...
        if ($eventModel->delete($id)) {
            ...
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la suppression de l\'événement.'];
        }
    ...
    public function listParticipations(): void {
        $this->auth->requireRole('admin');
        $viewPath = __DIR__ . '/../views/backoffice/participations_list.php';
        file_exists($viewPath) ? require_once $viewPath : http_response_code(200);
    }
    ...
    public function deleteParticipation(int $id): void {
        ...
        if ($participationModel->delete($id)) {
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Participation supprimée avec succès.'];
        } else {
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la suppression de la participation.'];
        }
        
        header('Location: index.php?page=participations_admin');
        exit;
    }

    public function createParticipation(): void {
        $this->auth->requireRole('admin');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Validation simple
            $errors = [];
            if (!$event_id) $errors['event_id'] = 'Événement requis';
            if (!$user_id) $errors['user_id'] = 'Participant requis';
            
            if (empty($errors)) {
                if ($participationModel->create([
                    'event_id' => $event_id,
                    'user_id' => $user_id,
                    'statut' => $statut
                ])) {
                    $_SESSION['flash'] = ['type' => 'success', 'message' => 'Participation créée avec succès.'];
                    header('Location: index.php?page=participations_admin');
                    exit;
                } else {
                ...
                $_SESSION['flash'] = ['type' => 'error', 'message' => 'Veuillez remplir tous les champs requis.'];
            }
        }
        
        $viewPath = __DIR__ . '/../views/backoffice/participation_form.php';
        require_once $viewPath;
    }

    public function updateParticipation(int $id): void {
        $this->auth->requireRole('admin');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($participationModel->update($id, ['statut' => $statut])) {
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Participation modifiée avec succès.'];
                header('Location: index.php?page=participations_admin');
                exit;
            } else {
                $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la modification.'];
            }
        }
    ...
    public function listSponsors(): void {
        ...
        // Nettoyer les variables de session du formulaire
        ...
        $sql = "SELECT * FROM sponsors ORDER BY nom ASC";
        $sponsors = $db->query($sql, []);
        if (!is_array($sponsors)) {
            $sponsors = [];
        }
        
        $viewPath = __DIR__ . '/../views/backoffice/sponsors_list.php';
        if (file_exists($viewPath)) {
            require_once $viewPath;
        } else {
            echo "Vue non trouvée: " . htmlspecialchars($viewPath);
        }
    }
    ...
    public function createSponsor(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($_SESSION['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
                $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur de sécurité. Veuillez réessayer.'];
            ...
            // Validation
            $errors = $this->validateSponsorData($_POST);
            ...
            if (empty($errors)) {
                ...
                if ($sponsorModel->create($data)) {
                    $_SESSION['flash'] = ['type' => 'success', 'message' => 'Sponsor créé avec succès.'];
        ...
        $_SESSION['sponsor_form_mode'] = 'create';
        $_SESSION['sponsor_form_data'] = null;
        
        $viewPath = __DIR__ . '/../views/backoffice/sponsor_form.php';
        require_once $viewPath;
    }
    ...
    public function editSponsor(int $id): void {
        ...
        if (!$sponsor) {
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Sponsor introuvable.'];
            header('Location: index.php?page=sponsors_admin');
            exit;
        }
        
        // Passer les données via SESSION pour être sûr qu'elles arrivent
        $_SESSION['sponsor_form_mode'] = 'edit';
        $_SESSION['sponsor_form_data'] = $sponsor;
        $_SESSION['sponsor_form_id'] = $id;
        
        $viewPath = __DIR__ . '/../views/backoffice/sponsor_form.php';
        require_once $viewPath;
    }

    public function updateSponsor(int $id): void {
        $this->auth->requireRole('admin');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                if ($sponsorModel->update($id, $data)) {
                    $_SESSION['flash'] = ['type' => 'success', 'message' => 'Sponsor modifié avec succès.'];
                    header('Location: index.php?page=sponsors_admin');
                    exit;
                } else {
                    $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la modification.'];
        ...
        // Charger le sponsor pour affichage
        $sponsor = $sponsorModel->getById($id);
        if (!$sponsor) {
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Sponsor introuvable.'];
            header('Location: index.php?page=sponsors_admin');
    ...
    private function validateSponsorData(array $data): array {
        $errors = [];

        // Nom (3-255 caractères)
        if (empty($data['nom'])) {
            $errors['nom'] = 'Le nom du sponsor est requis.';
        ...
        // Secteur (2-100 caractères)
        if (empty($data['secteur'])) {
            $errors['secteur'] = 'Le secteur est requis.';
        } elseif (strlen($data['secteur']) < 2) {
            $errors['secteur'] = 'Le secteur doit contenir au moins 2 caractères.';
        } elseif (strlen($data['secteur']) > 100) {
            $errors['secteur'] = 'Le secteur ne doit pas dépasser 100 caractères.';
        }

        // Budget (nombre positif)
        if (empty($data['budget'])) {
            $errors['budget'] = 'Le budget est requis.';
        } elseif (!is_numeric($data['budget']) || floatval($data['budget']) < 0) {
            $errors['budget'] = 'Le budget doit être un nombre positif.';
        } elseif (floatval($data['budget']) > 999999999) {
            $errors['budget'] = 'Le budget est trop élevé.';
        }
        ...
        // Téléphone (7-20 caractères)
        if (empty($data['telephone'])) {
            $errors['telephone'] = 'Le téléphone est requis.';
        } elseif (strlen($data['telephone']) < 7 || strlen($data['telephone']) > 20) {
            $errors['telephone'] = 'Le téléphone doit contenir entre 7 et 20 caractères.';
        }

        // Site Web (optionnel mais format valide si rempli)
        if (!empty($data['site_web']) && !filter_var($data['site_web'], FILTER_VALIDATE_URL)) {
            $errors['site_web'] = 'L\'URL n\'est pas valide.';
        }
        ...
        // Contact Email (optionnel mais format valide si rempli)
        if (!empty($data['contact_email']) && !filter_var($data['contact_email'], FILTER_VALIDATE_EMAIL)) {
            $errors['contact_email'] = 'L\'email du contact n\'est pas valide.';
        }
    ...
    public function deleteSponsor(int $id): void {
        ...
        if ($sponsorModel->delete($id)) {
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Sponsor supprimé avec succès.'];
        } else {
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Erreur lors de la suppression du sponsor.'];
        }
