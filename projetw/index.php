...
if ($page === 'get_face_photo') {
    ...
    if (!$userFace || empty($userFace['face_photo'])) {
        echo json_encode(['success' => false, 'message' => 'Aucune photo enregistrée']);
        exit;
    }
...
if ($page === 'api_reply_update') {
    $front->apiReplyUpdate();
    exit;
}
...
switch ($page) {
    ...
    case 'modifier_rendez_vous':
        patientOnly();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') $patientCtrl->updateRendezVous();
        break;
    ...
    // ─── Ordonnances depuis RDV ─────────────────────────────────
    case 'creer_ordonnance_rdv':
        medecinOnly();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') $ordonnanceCtrl->storeFromRendezVous();
        break;

    case 'modifier_ordonnance_rdv':
        medecinOnly();
        if ($_SERVER['REQUEST_METHOD'] === 'POST') $ordonnanceCtrl->updateFromRendezVous();
        break;
    ...
    case 'api_ordonnance':
        requireLogin();
        if ($action === 'get' && $id) $ordonnanceCtrl->apiGet($id);
        break;
    ...
    case 'disponibilites_admin':
        adminOnly();
        if ($action === 'create') {
            $_SERVER['REQUEST_METHOD'] === 'POST' ? $adminCtrl->createDisponibilite() : $adminCtrl->showCreateDisponibilite();
        } elseif ($action === 'edit' && $id) {
            $_SERVER['REQUEST_METHOD'] === 'POST' ? $adminCtrl->updateDisponibilite($id) : $adminCtrl->editDisponibilite($id);
        } elseif ($action === 'delete' && $id) {
