...
// Modèles optionnels
$optionalModels = [
    'RendezVous', 'Disponibilite', 'Event', 'EventAvance', 'Ordonnance',
...
$optionalControllers = [
    'RendezVousController', 'EventController', 'EventAvanceController',
    'ProduitController', 'OrdonnanceController', 'DisponibiliteController',
    'PharmacieController', 'CategorieController',
];
...
$publicPages = [
    ...
    'evenements', 'detail_evenement', 'mes_inscriptions_evenements', 'contact', 'about', 'sponsors',
];
...
switch ($page) {
    ...
    case 'mes_inscriptions_evenements':
...
case 'articles_admin':
    ...
    case 'evenements_admin':
        adminOnly();
        if ($action === 'create') {
            $adminCtrl->createEvent();
        } elseif ($action === 'edit' && $id) {
            $adminCtrl->updateEvent($id);
        } elseif ($action === 'delete' && $id) {
    ...
    // ─── Événements Avancé (DOCTIME integration) ─────────────
    case 'evenements_avance_admin':
        adminOnly();
        if (class_exists('EventAvanceController')) {
            $eventAvanceCtrl = new EventAvanceController();
            if ($action === 'stats') {
                $eventAvanceCtrl->stats();
            } elseif ($action === 'recherche') {
                $eventAvanceCtrl->recherche();
            } elseif ($action === 'exportCsv') {
                $eventAvanceCtrl->exportCsv();
            } elseif ($action === 'exportPreview') {
                $eventAvanceCtrl->exportPreview();
            } else {
                $eventAvanceCtrl->index();
            }
        } else {
            $front->page404();
        }
    ...
    // ─── Participations (backoffice) ──────────────────────────
    case 'participations_admin':
        adminOnly();
        if ($action === 'create') {
            $adminCtrl->createParticipation();
        } elseif ($action === 'update' && $id) {
            $adminCtrl->updateParticipation($id);
        } elseif ($action === 'delete' && $id) {
            $adminCtrl->deleteParticipation($id);
        } else {
            $adminCtrl->listParticipations();
        }
        break;
...
case 'sponsors_admin':
    adminOnly();
    if ($action === 'create') {
        $adminCtrl->createSponsor();
    } elseif ($action === 'edit' && $id) {
        $adminCtrl->editSponsor($id);
    } elseif ($action === 'update' && $id) {
        $adminCtrl->updateSponsor($id);
    } elseif ($action === 'delete' && $id) {
        $adminCtrl->deleteSponsor($id);
    } else {
        $adminCtrl->listSponsors();
    }
    break;
    ...
    // ─── JOINTURE — Recherche produits par catégorie ──────────
    case 'search_products_category':
    case 'jointure_produits':
        require __DIR__ . '/views/frontoffice/showProduitsByCategorie.html';
        break;

    // ─── JOINTURE — Recherche replies par article ────────────
    case 'search_replies_article':
    case 'jointure_articles':
        require __DIR__ . '/views/frontoffice/showRepliesByArticle.html';
        break;
