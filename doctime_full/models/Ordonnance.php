...
class Ordonnance {
    ...
    private function getColumns(): array {
        ...
        try {
            ...
            if (!$conn) return $this->columnsCache = [];
            $stmt = $conn->query("SHOW COLUMNS FROM ordonnances");
...
public function create(array $data): ?int {
    ...
    // ─────────────────────────────────────────
    //  Filtres Patient (utilisés par OrdonnanceController)
    // ─────────────────────────────────────────
    public function getActiveByPatient(int $patientId): array {
        try {
            $sql = "SELECT o.*,
                           CONCAT(u_medecin.prenom, ' ', u_medecin.nom) AS medecin_nom,
                           m.specialite
            ...
            if ($this->hasColumn('archivé')) {
                $sql .= " AND (o.archivé = 0 OR o.archivé IS NULL)";
            }
            if ($this->hasColumn('date_expiration')) {
                $sql .= " AND (o.date_expiration IS NULL OR o.date_expiration >= CURDATE())";
            }

            $sql .= " ORDER BY o.date_ordonnance DESC";
            return $this->db->query($sql, $params);
        } catch (Exception $e) {
            error_log('Erreur Ordonnance::getActiveByPatient - ' . $e->getMessage());
            return $this->getAllByPatient($patientId);
        }
    }

    public function getExpiredByPatient(int $patientId): array {
        try {
            if (!$this->hasColumn('date_expiration') && !$this->hasColumn('date_validite')) {
                return [];
            }

            $dateCol = $this->hasColumn('date_expiration') ? 'date_expiration' : 'date_validite';
            ...
            $sql = "SELECT o.*,
                    ...
                    LEFT JOIN medecins m ON o.medecin_id = m.user_id
                    WHERE o.patient_id = :patient_id
                      AND o.$dateCol IS NOT NULL
                      AND o.$dateCol < CURDATE()";
            ...
            error_log('Erreur Ordonnance::getExpiredByPatient - ' . $e->getMessage());
            return [];
        }
    }

    public function getArchivedByPatient(int $patientId): array {
        try {
            if (!$this->hasColumn('archivé')) return [];
            ...
            error_log('Erreur Ordonnance::getArchivedByPatient - ' . $e->getMessage());
            return [];
        }
    }

    // ─────────────────────────────────────────
    //  Filtres Médecin (utilisés par OrdonnanceController)
    // ─────────────────────────────────────────
    public function getByMedecinAndPatient(int $medecinId, int $patientId): array {
        try {
            $sql = "SELECT o.*,
                           CONCAT(u_patient.prenom, ' ', u_patient.nom) AS patient_nom,
                           u_patient.email AS patient_email
            ...
            return $this->db->query($sql, ['medecin_id' => $medecinId, 'patient_id' => $patientId]);
        } catch (Exception $e) {
            error_log('Erreur Ordonnance::getByMedecinAndPatient - ' . $e->getMessage());
            return [];
        }
    }
    ...
    public function getTodayByMedecin(int $medecinId): array {
        try {
            $sql = "SELECT o.*,
                    ...
                    LEFT JOIN users u_patient ON o.patient_id = u_patient.id
                    WHERE o.medecin_id = :medecin_id
                      AND DATE(o.date_ordonnance) = CURDATE()
                    ORDER BY o.date_ordonnance DESC";

            return $this->db->query($sql, ['medecin_id' => $medecinId]);
        } catch (Exception $e) {
            error_log('Erreur Ordonnance::getTodayByMedecin - ' . $e->getMessage());
            return [];
        }
    }

    public function getRecentByMedecin(int $medecinId, int $days = 30): array {
        try {
            $days = max(1, $days);
            $sql = "SELECT o.*,
                           CONCAT(u_patient.prenom, ' ', u_patient.nom) AS patient_nom,
                    ...
                    LEFT JOIN users u_patient ON o.patient_id = u_patient.id
                    WHERE o.medecin_id = :medecin_id
                      AND o.date_ordonnance >= DATE_SUB(CURDATE(), INTERVAL :days DAY)
                    ORDER BY o.date_ordonnance DESC";

            return $this->db->query($sql, ['medecin_id' => $medecinId, 'days' => $days]);
        } catch (Exception $e) {
            error_log('Erreur Ordonnance::getRecentByMedecin - ' . $e->getMessage());
            return $this->getAllByMedecin($medecinId);
        }
    }
