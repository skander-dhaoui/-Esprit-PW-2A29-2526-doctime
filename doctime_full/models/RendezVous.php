...
public function getByPatient(int $patientId, string $statut = null, string $date = null): array {
    ...
    // ─────────────────────────────────────────
    //  Méthodes de listing utilisées par controllers
    // ─────────────────────────────────────────
        ...
        try {
            ...
            $sql = "SELECT rv.*, 
                    ...
                    WHERE rv.patient_id = :patient_id
                      ...
                      AND DATE(rv.date_rendezvous) >= CURDATE()
                    ORDER BY rv.date_rendezvous ASC, rv.heure_rendezvous ASC";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':patient_id' => $patientId]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            error_log('Erreur RendezVous::getUpcomingByPatient - ' . $e->getMessage());
            return [];
        }
    }

    public function getPastByPatient(int $patientId): array {
        try {
            $conn = $this->db->getConnection();
            $sql = "SELECT rv.*, 
                           CONCAT(u_patient.prenom, ' ', u_patient.nom) AS patient_nom,
                    ...
                    LEFT JOIN medecins m ON rv.medecin_id = m.user_id
                    WHERE rv.patient_id = :patient_id
                      AND (rv.statut = 'terminé' OR DATE(rv.date_rendezvous) < CURDATE())
                    ORDER BY rv.date_rendezvous DESC, rv.heure_rendezvous DESC";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':patient_id' => $patientId]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            error_log('Erreur RendezVous::getPastByPatient - ' . $e->getMessage());
            return [];
        }
    }

    public function getCancelledByPatient(int $patientId): array {
        return $this->getByPatient($patientId, 'annulé');
    }

    public function getAllByMedecin(int $medecinId): array {
        return $this->getByMedecin($medecinId);
    }

    public function getTodayByMedecin(int $medecinId): array {
        return $this->getByMedecin($medecinId, null, date('Y-m-d'));
    }
    ...
    public function getUpcomingByMedecin(int $medecinId): array {
        try {
            ...
            error_log('Erreur RendezVous::getUpcomingByMedecin - ' . $e->getMessage());
            return [];
        }
    }

    public function getPastByMedecin(int $medecinId): array {
        try {
            $conn = $this->db->getConnection();
            $sql = "SELECT rv.*, 
                           CONCAT(u_patient.prenom, ' ', u_patient.nom) AS patient_nom,
            ...
            error_log('Erreur RendezVous::getPastByMedecin - ' . $e->getMessage());
            return [];
        }
    }

    public function isAvailable(int $medecinId, string $dateRdv, string $heureRdv, ?int $excludeId = null): bool {
        try {
            $conn = $this->db->getConnection();
            $sql = "SELECT COUNT(*) FROM rendez_vous
                    WHERE medecin_id = :medecin_id
                      AND date_rendezvous = :date_rendezvous
                      AND heure_rendezvous = :heure_rendezvous
                      AND statut NOT IN ('annulé', 'terminé')";
            $params = [
                ':medecin_id' => $medecinId,
                ':date_rendezvous' => $dateRdv,
            ...
            error_log('Erreur RendezVous::isAvailable - ' . $e->getMessage());
            return false;
        }
    }

    public function getAvailabilitiesByMedecin(int $medecinId, string $date): array {
        try {
            $conn = $this->db->getConnection();
            $map = [
                'Monday' => 'Lundi',
            ...
            $jour = $map[date('l', strtotime($date))] ?? '';
            if ($jour === '') return [];

            $sql = "SELECT d.* FROM disponibilites d
                    WHERE d.medecin_id = :medecin_id
                      AND d.jour_semaine = :jour
                      AND d.actif = 1
                    ORDER BY d.heure_debut ASC";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':medecin_id' => $medecinId, ':jour' => $jour]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            error_log('Erreur RendezVous::getAvailabilitiesByMedecin - ' . $e->getMessage());
            return [];
        }
    }
    ...
    private function ensureWaitlistTable(): void {
        try {
            ...
            // Compatibilité montées de version: ajoute colonnes de notification si absentes.
            $colsStmt = $conn->query("SHOW COLUMNS FROM rendez_vous_waitlist");
            $cols = $colsStmt ? array_map(static fn($r) => (string)($r['Field'] ?? ''), $colsStmt->fetchAll(PDO::FETCH_ASSOC)) : [];
            if (!in_array('converted_at', $cols, true)) {
                $conn->exec("ALTER TABLE rendez_vous_waitlist ADD COLUMN converted_at DATETIME NULL");
            }
            if (!in_array('notified_patient', $cols, true)) {
                $conn->exec("ALTER TABLE rendez_vous_waitlist ADD COLUMN notified_patient TINYINT(1) NOT NULL DEFAULT 0");
            }
        } catch (Exception $e) {
            error_log('Erreur RendezVous::ensureWaitlistTable - ' . $e->getMessage());
        }
    }

    public function addToWaitlist(int $patientId, int $medecinId, string $dateRendezvous, string $heureRendezvous, string $motif = ''): bool {
        try {
            ...
            $stmt = $conn->prepare("
                ...
                VALUES (:patient_id, :medecin_id, :date_rendezvous, :heure_rendezvous, :motif, 'pending')
            ");
            return $stmt->execute([
                ':patient_id' => $patientId,
                ':medecin_id' => $medecinId,
            ...
            error_log('Erreur RendezVous::addToWaitlist - ' . $e->getMessage());
            return false;
        }
    }

    public function fillWaitlistForFreedSlot(int $medecinId, string $dateRendezvous, string $heureRendezvous): ?int {
        try {
            if ($medecinId <= 0 || $dateRendezvous === '' || $heureRendezvous === '') {
                return null;
            }
            $this->ensureWaitlistTable();
            $conn = $this->db->getConnection();
            ...
            $candidate = $pick->fetch(PDO::FETCH_ASSOC);
            ...
            $insert = $conn->prepare("
                INSERT INTO rendez_vous (patient_id, medecin_id, date_rendezvous, heure_rendezvous, motif, statut, created_at, updated_at)
                VALUES (:patient_id, :medecin_id, :date_rendezvous, :heure_rendezvous, :motif, 'en_attente', NOW(), NOW())
            ");
            $insert->execute([
                ...
                ':date_rendezvous' => $dateRendezvous,
                ':heure_rendezvous' => $heureRendezvous,
                ':motif' => (string)($candidate['motif'] ?? 'Consultation'),
            ]);
            $newRdvId = (int)$conn->lastInsertId();

            $update = $conn->prepare("
                UPDATE rendez_vous_waitlist
                SET status = 'converted',
                    converted_rdv_id = :rdv_id,
                    converted_at = NOW(),
                    notified_patient = 0,
                    updated_at = NOW()
                WHERE id = :id
            ...
            error_log('Erreur RendezVous::fillWaitlistForFreedSlot - ' . $e->getMessage());
            return null;
        }
    }

    public function popWaitlistPromotionNotifications(int $patientId): array {
        try {
            $this->ensureWaitlistTable();
            $conn = $this->db->getConnection();
            ...
            error_log('Erreur RendezVous::popWaitlistPromotionNotifications - ' . $e->getMessage());
            return [];
        }
    }
}
