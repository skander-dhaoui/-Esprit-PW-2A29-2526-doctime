...
class EventAvance {
    ...
    private function hasSponsorColumn(): bool {
        ...
        try {
            $stmt = $this->pdo->prepare("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'events' AND COLUMN_NAME = 'sponsor_id'");
            $stmt->execute();
    ...
    private function getSponsorSelect(): string {
        if ($this->hasSponsorColumn()) {
            return "s.nom AS sponsor_nom, s.niveau AS sponsor_niveau";
        }

        return "NULL AS sponsor_nom, NULL AS sponsor_niveau";
    }
    ...
    public function recherche(array $filtres): array {
        ...
        if (!empty($filtres['sponsor_id']) && $this->hasSponsorColumn()) {
