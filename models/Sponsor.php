...
class Sponsor {
    ...
    private function columnExists(string $table, string $column): bool {
        try {
            ...
            $result = $this->db->query($sql, ['table' => $table, 'column' => $column]);
    ...
    public function getAll(int $offset = 0, int $limit = 20, string $filter = 'actif', string $search = '', string $secteur = ''): array {
        try {
            ...
            $hasStatutColumn = $this->columnExists('sponsors', 'statut');
            if ($hasStatutColumn && $filter !== 'all') {
                $where .= " AND s.statut = :statut";
            }

            // Vérifier les colonnes de recherche
            $hasEmailColumn = $this->columnExists('sponsors', 'email');
            $hasTelephoneColumn = $this->columnExists('sponsors', 'telephone');
            $hasSecteurColumn = $this->columnExists('sponsors', 'secteur');

            if (!empty($search) && ($hasEmailColumn || $hasTelephoneColumn)) {
                $searchConditions = [];
                $searchConditions[] = "s.nom LIKE :search";
                if ($hasEmailColumn) $searchConditions[] = "s.email LIKE :search";
                if ($hasTelephoneColumn) $searchConditions[] = "s.telephone LIKE :search";
                $where .= " AND (" . implode(" OR ", $searchConditions) . ")";
            }

            if (!empty($secteur) && $hasSecteurColumn) {
