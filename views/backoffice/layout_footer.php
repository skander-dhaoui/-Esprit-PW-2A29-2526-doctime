...
document.addEventListener('DOMContentLoaded', function () {
    const PALETTE = ['#1a7fa8','#1db88e','#7c3aed','#a855f7','#f59e0b','#ef4444','#06b6d4','#10b981','#f97316'];
    ...
    if (document.getElementById('chartMontantNiveau') && typeof niveauLabels !== 'undefined') {
        const levelColors = { 'Bronze':'#cd7f32','Argent':'#9ca3af','Or':'#f59e0b','Platine':'#06b6d4' };
        new Chart(document.getElementById('chartMontantNiveau'), {
            type: 'bar',
            data: { labels: niveauLabels, datasets: [{ label: 'Montant (TND)', data: niveauData, backgroundColor: niveauLabels.map(n => levelColors[n] || '#1a7fa8'), borderRadius: 8, borderSkipped: false }] },
