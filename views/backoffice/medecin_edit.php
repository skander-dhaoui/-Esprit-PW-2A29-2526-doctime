...
if (!isset($medecin) || !is_array($medecin)) {
    $_SESSION['flash'] = ['type' => 'error', 'message' => 'Médecin introuvable.'];
    header('Location: index.php?page=medecins_admin'); exit;
}
...
<head>
    <meta charset="UTF-8">
    <title>Modifier un médecin - MediConnect</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .section-title{font-size:13px;font-weight:700;color:#1a2035;text-transform:uppercase;letter-spacing:.5px;border-bottom:2px solid #e2e8f0;padding-bottom:8px;margin:0 0 18px;display:flex;align-items:center;gap:8px}
        .section-title+.section-title,.section-title~.section-title{margin-top:24px}
        ...
        .medecin-banner{background:linear-gradient(135deg,#1a2035,#0fa99b);border-radius:12px;padding:20px 24px;margin-bottom:20px;color:white;display:flex;align-items:center;gap:16px}
        .medecin-avatar{width:54px;height:54px;background:rgba(255,255,255,.15);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;flex-shrink:0}
        .medecin-banner h5{margin:0;font-size:17px;font-weight:700}
        .medecin-banner small{opacity:.8;font-size:13px}
        hr.my-4{border-color:#e2e8f0}
    </style>
</head>
<body>
<div class="sidebar">
...
<div class="main-content">
    <div class="page-header">
        ...
        <a href="index.php?page=medecins_admin" class="btn btn-secondary btn-sm">
            <i class="fas fa-arrow-left me-1"></i> Retour
        </a>
    </div>

    <div class="medecin-banner">
        <div class="medecin-avatar"><i class="fas fa-user-md"></i></div>
        <div>
            <h5>Dr. <?= htmlspecialchars(($medecin['prenom'] ?? '') . ' ' . ($medecin['nom'] ?? '')) ?></h5>
            <small><?= htmlspecialchars($medecin['specialite'] ?? 'Spécialité non renseignée') ?> &bull; #<?= $medecin['id'] ?></small>
        </div>
    </div>
    ...
    <div class="content-card">
        <form method="POST" action="index.php?page=medecins_admin&action=edit&id=<?= $medecin['id'] ?>" novalidate id="medecinEditForm">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="nom" name="nom" class="form-control"
                           value="<?= htmlspecialchars($old['nom'] ?? $medecin['nom'] ?? '') ?>"
                           placeholder="Dupont">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="prenom" name="prenom" class="form-control"
                           value="<?= htmlspecialchars($old['prenom'] ?? $medecin['prenom'] ?? '') ?>"
                           placeholder="Jean">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="email" id="email" name="email" class="form-control"
                           value="<?= htmlspecialchars($old['email'] ?? $medecin['email'] ?? '') ?>">
                    <div class="invalid-feedback" id="email-error"></div>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Téléphone</label>
                    <input type="tel" id="telephone" name="telephone" class="form-control"
                           value="<?= htmlspecialchars($old['telephone'] ?? $medecin['telephone'] ?? '') ?>"
            ...
            <div class="section-title" style="margin-top:24px"><i class="fas fa-stethoscope"></i> Informations professionnelles</div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="specialite" name="specialite" class="form-control"
                           value="<?= htmlspecialchars($old['specialite'] ?? $medecin['specialite'] ?? '') ?>"
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" id="numero_ordre" name="numero_ordre" class="form-control"
                           value="<?= htmlspecialchars($old['numero_ordre'] ?? $medecin['numero_ordre'] ?? '') ?>">
                    <div class="invalid-feedback" id="numero_ordre-error"></div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label class="form-label">Tarif consultation (DT)</label>
                    <input type="number" id="consultation_prix" name="consultation_prix" class="form-control"
                           step="1" min="0"
                           value="<?= htmlspecialchars($old['consultation_prix'] ?? $medecin['consultation_prix'] ?? '') ?>"
                           placeholder="50">
                    <div class="invalid-feedback" id="tarif-error"></div>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Années d'expérience</label>
                    <input type="number" id="annee_experience" name="annee_experience" class="form-control"
                           min="0"
                           value="<?= htmlspecialchars($old['annee_experience'] ?? $medecin['annee_experience'] ?? '') ?>"
                           placeholder="0">
                    <div class="invalid-feedback" id="experience-error"></div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="statut" class="form-select">
                        <?php $st = $old['statut'] ?? $medecin['statut'] ?? 'actif'; ?>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <textarea name="cabinet_adresse" class="form-control" rows="2"><?= htmlspecialchars($old['cabinet_adresse'] ?? $medecin['cabinet_adresse'] ?? '') ?></textarea>
                </div>
            </div>
            ...
            <div class="d-flex gap-3 flex-wrap align-items-center">
                ...
                <a href="index.php?page=medecins_admin&action=delete&id=<?= $medecin['id'] ?>"
...
<script>
document.getElementById('medecinEditForm').addEventListener('submit', function(e) {
    let valid = true;
    ...
    if (!specialite) err('specialite-error', 'specialite', 'La spécialité est obligatoire.');
