<?php
// Les variables $error et $old sont passées par AuthController::showRegister()
?>
<!DOCTYPE html>
<html lang="fr">
<head>
        ...
        <div class="register-body">
            ...
            <?php if (!empty($error)): ?>
                <div class="alert alert-danger alert-php">
                    ...
                    <?= $error /* peut contenir du HTML (br) */ ?>
                </div>
            <?php endif; ?>
                ...
                <div id="medecinFields" class="medecin-fields">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="numero_ordre" id="numeroOrdre" class="form-control"
    ...
    <script>
        ...
        document.getElementById('registerForm').addEventListener('submit', function (e) {
            ...
            if (!nom || !prenom || !email || !tel || !password || !confirm) {
                e.preventDefault(); showError('Veuillez remplir tous les champs obligatoires.'); return;
            }
            const pv = validatePassword(password);
            if (!pv.length || !pv.upper || !pv.number) {
                e.preventDefault(); showError('Le mot de passe ne respecte pas les critères de sécurité.'); return;
            }
            if (password !== confirm) {
                e.preventDefault(); showError('Les mots de passe ne correspondent pas.'); return;
            }
            if (!terms) {
                e.preventDefault(); showError("Vous devez accepter les conditions d'utilisation."); return;
            }
            if (role === 'medecin') {
                const s = document.getElementById('specialite').value;
                const n = document.getElementById('numeroOrdre').value;
                if (!s) { e.preventDefault(); showError('Veuillez sélectionner votre spécialité.'); return; }
                if (!n) { e.preventDefault(); showError("Veuillez saisir votre numéro d'ordre."); return; }
            }
            // Tout est OK → soumission PHP normale
        });
