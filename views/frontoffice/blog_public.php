...
<div class="blog-header">
    <div class="container">
        ...
        <!-- Barre de recherche -->
        <form class="d-flex justify-content-center mt-4" onsubmit="event.preventDefault(); searchArticles();">
            <input type="text" id="searchInputTop" class="form-control me-2" style="max-width:350px;" placeholder="Rechercher un article...">
            <button class="btn btn-primary" type="button" onclick="searchArticles(true)"><i class="fas fa-search me-1"></i>Rechercher</button>
        </form>
    </div>
</div>
...
<div style="background: linear-gradient(135deg, #e8f4f8 0%, #f0f9e8 100%); padding: 28px 0; margin-bottom: 32px; border-bottom: 2px solid #4CAF50;">
    <div class="container">
        ...
        <div class="row g-3" style="justify-content: center;">
            ...
            <div class="col-md-5">
                <a href="index.php?page=jointure_produits" style="text-decoration: none;">
                    <div style="
                        background: white;
                        border-radius: 12px;
                        padding: 20px;
            ...
            <div class="col-md-5">
                <a href="index.php?page=jointure_articles" style="text-decoration: none;">
                    <div style="
                        background: white;
                        border-radius: 12px;
                        padding: 20px;
...
<!-- ═══════════════════════════════════════════════════════ -->
<!--     BARRE DE RECHERCHE JOINTURE ARTICLES/REPLIES      -->
<!-- ═══════════════════════════════════════════════════════ -->
<div style="background: white; padding: 28px 0; margin-bottom: 32px; border-top: 2px solid #e9ecef; border-bottom: 2px solid #e9ecef;">
    <div class="container">
        <div style="background: linear-gradient(135deg, #f0f7ff 0%, #f9e8f0 100%); border-radius: 12px; padding: 24px; border-left: 4px solid #4CAF50;">
            <h4 style="color: #1a2035; margin-bottom: 16px;">
                <i class="fas fa-comments me-2" style="color: #4CAF50;"></i>Chercher les commentaires d'un article
            </h4>
            <form method="POST" action="index.php?page=blog_public" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;">
                <div style="flex: 1; min-width: 250px;">
                    <label style="display: block; font-size: 12px; font-weight: 600; color: #666; margin-bottom: 6px; text-transform: uppercase;">Sélectionner un article</label>
                    <select name="article_search" style="width: 100%; padding: 10px 12px; border: 1.5px solid #e9ecef; border-radius: 8px; font-size: 14px; background: white; color: #333;" required>
                        <option value="">-- Choisir un article --</option>
                        <?php
                        // Récupérer tous les articles avec comptage des replies
                        require_once __DIR__ . '/../../models/Article.php';
                        $articleModel = new Article();
                        $articles = $articleModel->getArticlesWithReplyCount();
                        foreach ($articles as $art):
                        ?>
                            <option value="<?= $art['id'] ?>">
                                <?= htmlspecialchars($art['titre']) ?> (<?= $art['nombre_replies'] ?> commentaire<?= $art['nombre_replies'] != 1 ? 's' : '' ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <button type="submit" style="background: linear-gradient(135deg, #4CAF50 0%, #2A7FAA 100%); color: white; border: none; padding: 10px 28px; border-radius: 8px; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.3s;" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">
                    <i class="fas fa-search me-2"></i>Afficher les commentaires
                </button>
            </form>

            <!-- Affichage des commentaires si un article est sélectionné -->
            <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['article_search']) && !empty($_POST['article_search'])): ?>
                <?php 
                $selectedArticleId = (int)$_POST['article_search'];
                $selectedArticle = $articleModel->getArticleWithReplies($selectedArticleId);
                
                if ($selectedArticle && !empty($selectedArticle['replies'])): 
                ?>
                    <div style="margin-top: 24px; padding-top: 24px; border-top: 2px solid #e9ecef;">
                        <h5 style="color: #1a2035; margin-bottom: 16px;">
                            <i class="fas fa-comments me-2" style="color: #4CAF50;"></i>Commentaires de: <strong><?= htmlspecialchars($selectedArticle['article']['titre']) ?></strong>
                        </h5>
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            <?php foreach ($selectedArticle['replies'] as $reply): ?>
                                <div style="background: #f8f9fa; border-left: 4px solid #4CAF50; padding: 16px; border-radius: 6px;">
                                    <div style="font-weight: 600; color: #1a2035; margin-bottom: 4px;">
                                        <i class="fas fa-user-circle me-2" style="color: #4CAF50;"></i><?= htmlspecialchars($reply['auteur'] ?? 'Anonyme') ?>
                                    </div>
                                    <div style="font-size: 12px; color: #999; margin-bottom: 8px;">
                                        <i class="fas fa-calendar-alt me-1"></i><?= date('d/m/Y H:i', strtotime($reply['date_reply'])) ?>
                                    </div>
                                    <div style="color: #555; font-size: 14px;">
                                        <?php if ($reply['type_reply'] === 'text'): ?>
                                            <?= nl2br(htmlspecialchars($reply['contenu_text'] ?? $reply['text_content'] ?? '')) ?>
                                        <?php elseif ($reply['type_reply'] === 'photo'): ?>
                                            <em>📸 Commentaire avec photo</em>
                                            <?php if ($reply['contenu_text'] || $reply['text_content']): ?>
                ...
                <?php elseif ($selectedArticle): ?>
                    <div style="margin-top: 24px; padding: 16px; background: #e8f5e9; border-radius: 6px; color: #2e7d32; border-left: 4px solid #4CAF50;">
                        <i class="fas fa-info-circle me-2"></i><strong><?= htmlspecialchars($selectedArticle['article']['titre']) ?></strong> n'a pas encore de commentaires.
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="sidebar-card d-flex flex-column gap-2">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h5 class="mb-0"><i class="fas fa-plus-circle me-2"></i>Publier un article</h5>
                    <a href="index.php?page=blog_public" class="btn btn-outline-primary btn-sm" title="Voir le blog">
...
// Recherche locale (supporte barre du haut et sidebar)
function searchArticles(fromTop = false) {
    const q = (fromTop ? $('searchInputTop').value : $('searchInput').value).toLowerCase().trim();
...
if ($('searchInput')) $('searchInput').addEventListener('keypress', e => { if (e.key === 'Enter') searchArticles(); });
if ($('searchInputTop')) $('searchInputTop').addEventListener('keypress', e => { if (e.key === 'Enter') searchArticles(true); });
