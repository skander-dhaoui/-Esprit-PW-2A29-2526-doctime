...
<head>
    ...
    <style>
        ...
        /* ARTICLE CARD */
        .art-card{background:#fff;border-radius:16px;padding:28px;margin-bottom:24px;box-shadow:0 4px 16px rgba(0,0,0,.07);border-left:4px solid #2A7FAA;transition:.3s}
        ...
        .btn-read{background:#2A7FAA;color:#fff;border:none;border-radius:25px;padding:7px 18px;font-size:13px;cursor:pointer;transition:.2s;text-decoration:none}
        .btn-read:hover{background:#1e5f80;color:#fff}
        .art-actions{display:flex;gap:6px}
        .btn-sm-edit{background:#e3f2fd;color:#1565c0;border:none;border-radius:20px;padding:5px 12px;font-size:12px;cursor:pointer;font-weight:600;transition:.2s}
        .btn-sm-edit:hover{background:#1565c0;color:#fff}
        .btn-sm-del{background:#fce4ec;color:#c62828;border:none;border-radius:20px;padding:5px 12px;font-size:12px;cursor:pointer;font-weight:600;transition:.2s}
        ...
        .back-btn{display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.2);color:#fff;border:none;border-radius:25px;padding:8px 18px;font-size:13px;cursor:pointer;margin-bottom:16px;transition:.2s}
        ...
        .comment-item{border-bottom:1px solid #f0f4f8;padding:16px 0;display:flex;gap:14px}
        ...
        .form-group-custom input,.form-group-custom textarea,.form-group-custom select{width:100%;padding:10px 13px;border:1.5px solid #e9ecef;border-radius:10px;font-size:13px;font-family:inherit;outline:none;transition:.15s;background:#f8f9fc;color:#1a2035}
        ...
        .modal-box{background:#fff;border-radius:20px;width:100%;max-width:500px;max-height:90vh;overflow-y:auto;animation:pop .22s ease;box-shadow:0 20px 60px rgba(0,0,0,.2)}
...
<div class="container mb-5">
    <div class="row">
        ...
        <div class="col-lg-8" id="viewList">
            ...
            <?php endif; ?>
            <div id="articlesList">
                <div class="text-center py-5"><span class="spinner"></span><p class="mt-2 text-muted">Chargement...</p></div>
            </div>
        </div>
        ...
        <div class="col-lg-8" id="viewDetail" style="display:none">
            ...
            <!-- ACTIONS ARTICLE (si connecté et auteur) -->
            ...
            <div class="comment-form" id="commentForm">
                ...
                <div class="form-group-custom" id="nfText">
                    ...
                    <textarea id="newRepText" placeholder="Votre commentaire..."></textarea>
        ...
        <div class="col-lg-4" id="sidebarCol">
            <div class="sidebar-card">
                ...
                <div class="input-group">
                    <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Rechercher un article...">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
            <div class="sidebar-card" id="newArtBox">
                ...
                <div class="form-group-custom">
                    <label>Contenu *</label>
                    <textarea id="newArtContenu" placeholder="Contenu..." style="min-height:80px"></textarea>
...
<div class="overlay" id="ovEditArt">
    <div class="modal-box">
        ...
        <div class="mb">
            <div class="form-group-custom"><label>Titre *</label><input id="editArtTitre"><div class="err-msg" id="eEditArtTitre"></div></div>
            <div class="form-group-custom"><label>Contenu *</label><textarea id="editArtContenu"></textarea><div class="err-msg" id="eEditArtContenu"></div></div>
        </div>
        <div class="mf">
            <button class="btn-cancel" onclick="closeOv('ovEditArt')">Annuler</button>
            <button class="btn-save" id="btnEditArt" onclick="saveEditArt()"><i class="fas fa-save me-1"></i>Enregistrer</button>
        </div>
    </div>
</div>
...
<div class="overlay" id="ovEditRep">
    <div class="modal-box">
        ...
        <div class="mb">
            ...
            <div class="form-group-custom" id="efText"><label>Commentaire *</label><textarea id="editRepText"></textarea><div class="err-msg" id="eEditRepText"></div></div>
            <div class="form-group-custom" id="efEmoji" style="display:none"><label>Emoji *</label><input id="editRepEmoji"><div class="err-msg" id="eEditRepEmoji"></div></div>
            <div class="form-group-custom" id="efPhoto" style="display:none"><label>URL Photo *</label><input id="editRepPhoto"><div class="err-msg" id="eEditRepPhoto"></div></div>
        </div>
        <div class="mf">
            <button class="btn-cancel" onclick="closeOv('ovEditRep')">Annuler</button>
            <button class="btn-save" id="btnEditRep" onclick="saveEditRep()"><i class="fas fa-save me-1"></i>Enregistrer</button>
        </div>
    </div>
</div>
...
function clrFG(ids){ids.forEach(id=>{const e=$(id);if(e){e.value='';e.classList.remove('err')}})}
...
async function loadArticles(){
    $('articlesList').innerHTML='<div class="text-center py-5"><span class="spinner"></span></div>';
        ...
        <div class="art-card">
            ...
            <div class="art-footer">
                ...
                ${IS_LOGGED?`<div class="art-actions">
                    <button class="btn-sm-edit" onclick="openEditArtFromList(${a.id_article},event)"><i class="fas fa-edit me-1"></i>Modifier</button>
                    <button class="btn-sm-del" onclick="confirmDelArtFromList(${a.id_article},'${esc(a.titre).replace(/'/g,"\\'")}',event)"><i class="fas fa-trash me-1"></i>Supprimer</button>
                </div>`:''}
            </div>
        </div>`).join('');
}
...
async function openArticle(id){
    ...
    // Actions admin sur l'article
    if(IS_LOGGED){$('artOwnerActions').style.display=''}
    else{$('artOwnerActions').style.display='none'}
...
async function postComment(){
    ...
    if(type==='text'){contenu_text=$('newRepText').value.trim();if(!contenu_text){setErr('eNewRepText','Le texte est obligatoire.');$('newRepText').classList.add('err');ok=false}}
    else if(type==='emoji'){emoji=$('newRepEmoji').value.trim();if(!emoji){setErr('eNewRepEmoji',"L'emoji est obligatoire.");$('newRepEmoji').classList.add('err');ok=false}}
    else{photo=$('newRepPhoto').value.trim();if(!photo){setErr('eNewRepPhoto',"L'URL est obligatoire.");$('newRepPhoto').classList.add('err');ok=false}}
    if(!ok)return;
...
async function saveEditRep(){
    if(!IS_LOGGED||!editRepData)return;
    ...
    if(!ok)return;
    const btn=$('btnEditRep');btn.disabled=true;btn.innerHTML='<span class="spinner"></span>';
    const r=await apiPost({page:'api_reply',id},{id_article:editRepData.id_article,type_reply:type,contenu_text,emoji,photo,auteur:editRepData.auteur,_method:'PUT'});
    btn.disabled=false;btn.innerHTML='<i class="fas fa-save me-1"></i>Enregistrer';
    if(!r.success){toast(r.message||'Erreur','err');return}
    closeOv('ovEditRep');toast('Commentaire modifié !','ok');
    const r2=await apiGet({page:'api_article',id:currentArt.id_article});
    if(r2.success)renderComments(r2.replies||[]);
}
...
async function postArticle(){
    ...
    $('newArtTitre').value='';$('newArtContenu').value='';
    loadArticles();
}

// ═══════════════════════════════════════════════════════
//  MODIFIER UN ARTICLE (liste + détail)
// ═══════════════════════════════════════════════════════
let editArtId=null;
...
async function saveEditArt(){
    ...
    if(!ok)return;
    const btn=$('btnEditArt');btn.disabled=true;btn.innerHTML='<span class="spinner"></span>';
    const r=await apiPost({page:'api_article',id:editArtId},{titre,contenu,_method:'PUT'});
    btn.disabled=false;btn.innerHTML='<i class="fas fa-save me-1"></i>Enregistrer';
    if(!r.success){toast(r.message||'Erreur','err');return}
    closeOv('ovEditArt');toast('Article modifié !','ok');
    if(currentArt){
        // Recharger le détail
        const r2=await apiGet({page:'api_article',id:editArtId});
        if(r2.success){currentArt=r2.article;$('detailTitle').textContent=r2.article.titre;$('detailBody').innerHTML=esc(r2.article.contenu).replace(/\n/g,'<br>')}
    } else {
        loadArticles();
    }
}
